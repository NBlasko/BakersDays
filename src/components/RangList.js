...
class VotersList extends Component {
    ...
    render() {
        ...
        let sorted = bakersArray.sort(function(a, b) {
          ...
          var sum = 0;
          for(var i in  bakersArray) { sum += bakersArray[i].score; }
        return (
            <div>
        <Button color="danger" onClick={this.signOut}> Sign Out </Button>
        <Button color="info" onClick={this.deleteDB}>  {(this.state.show === false)? "Obrisi sve podatke. Oprezno! " : "Sakrij! "} </Button>
       <div> {(this.state.show === true) ? <Button className = "text-light bg-dark" onClick={this.deleteDBFinaly}> Klik i nema povratka! </Button> : null} </div> 
        <h1 className="text-primary"> Rang Lista </h1> Suma svih glasova je : {sum}
                {
                    visible.map(element =>
                        <div key={element.vote}> <code> Pekar br.{element.vote}</code> ima {element.score} {(element.score === 1)? "poen " : "poena "}
                            <Button className="btn btn-outline-danger btn-sm"  onClick={this.handleClick} id ={element.vote} > Umanji </Button>
                        </div>)
                }
            </div>
        );
